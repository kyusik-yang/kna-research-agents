...
def draft_article(round_num):
    ...
    **Figure (MANDATORY at least 1):**
    ...
    Coefficient plot showing AME of minsaeng status by policy domain.
    Labor: -15.7pp, Care: near zero, Welfare: -8.2pp, SmallBiz: -3.1pp.]}}}}
    \\caption{{Average Marginal Effects of Minsaeng Status on Committee Processing, by Policy Domain}}
    \\label{{fig:coef}}
    \\end{{figure}}

    Do NOT use TikZ for figures (compilation errors are common).
