...
1. **Sensitivity-band framing, not single-point**. The honest reporting is that the sponsorship shirking effect lies in the range [-0.77, -1.5] bills/month depending on date-anchoring choice, with the NEC-proxy anchor preferred because it is closer to the causal timing. A single-point headline invites specification-search criticism; a band framing converts the ambiguity into a documented robustness range. This is the standard qualification move in observational political-science writing and I recommend the PAP adopt it.
